...
table_list = [
                 "task_log",
                 "task_status_update",
                 "task_task_fill_problems",
                 "user_batch_task_fill_problem",
                 "user_task_batch",
                 "user_task_batch_tasks",
                 "barcode",
                 "batch_response",
                 "batch_response_task_fill_problem_list",
                 "task_view",   # give permission to task_view instead of task
                 "library",
                 "location",
                 "circulation_desk_locations",
                 "avatar",
                 "circulation_desk",
                 "user_account_circ_desks",
                 "user_account",
                 "task_fill_problem",
                 "task_problem",
                 "task_assignment"
...
def setup_database(config):
    ...
    with connection.cursor() as c:
        try:
            ...
            c.execute(sql.SQL('alter user {username} with encrypted password %(password)s').format(username=sql.Identifier(username_for_ro)), {'password': password_for_ro})
    ...
    with connection.cursor() as c:
        ...
        for table in table_list:
            ...
            if table == 'task':
                continue
            c.execute(sql.SQL(f'GRANT SELECT ON public.{table} TO' + '{username}').format(username=sql.Identifier(username_for_ro)))
